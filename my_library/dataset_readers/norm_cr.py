...
def normalize_file(clean_text):
    ...
    with open(CRCfg.raw_pos, 'r') as f:
        for line in f:
            line = clean_text(line.strip())
            X.append(line)
            y.append('__label__POS')

    with open(CRCfg.raw_neg, 'r') as f:
        for line in f:
            line = clean_text(line.strip())
            X.append(line)
    ...
    with open(CRCfg.train_norm_path, 'w', encoding='utf-8') as f:
        for i in range(len(X_train)):
            if len(X_train[i]) > 0:
                f.write(y_train[i] + '\t' + X_train[i] + '\n')

    with open(CRCfg.test_path, 'w', encoding='utf-8') as f:
