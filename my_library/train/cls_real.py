...
@TrainerBase.register("cls-real")
class ClsRealTrainer(TrainerBase):
    def __init__(
            self,
            model: Model,
    ...
    def feature_collection(self):
        ...
        with torch.no_grad():
            ...
            for batch in self.iterator(self.train_data, num_epochs=1, shuffle=False):
                batch = nn_util.move_to_device(batch, self._cuda_devices[0])
                output_dict = self.model(**batch)
                features = output_dict['features']
                train_labels.extend(batch['label'].data.cpu().numpy())
                train_features.extend(features.data.cpu().numpy())

            logger.info("### validation features ###")
            for batch in self.iterator(self._validation_data, num_epochs=1, shuffle=False):
                batch = nn_util.move_to_device(batch, self._cuda_devices[0])
                output_dict = self.model(**batch)
                features = output_dict['features']
                validation_labels.extend(batch['label'].data.cpu().numpy())
                validation_features.extend(features.data.cpu().numpy())
            ...
            for batch in self.iterator(self._test_data, num_epochs=1, shuffle=False):
                ...
                output_dict = self.model(**batch)
                features = output_dict['features']
                test_labels.extend(batch['label'].data.cpu().numpy())
                test_features.extend(features.data.cpu().numpy())
