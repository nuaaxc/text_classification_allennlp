...
class CRCfg(DirCfg):
    ...
    raw_pos = os.path.join(data_dir, 'custrev.pos')
    raw_neg = os.path.join(data_dir, 'custrev.neg')
...
class CRCfgBert(CRCfg):
    ...
    hp.batch_per_epoch = int(CRCfg.training_size[hp.file_ratio] / hp.batch_size) + 1


class CRCfgLSTM(CRCfg):
    model_name = 'lstm'
    hp = HP()
    hp.phase = DirCfg.phase_real_str
    hp.file_ratio = 1
